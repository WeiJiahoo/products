...
void MainWindow::get_data()
{
    ...
    if(ui->rf_btn->isChecked()){
        dump_rf_data(&imu);

        int slave_id=ui->rf_id_combobox->currentIndex();
        for(int i=0;i<4;i++)
            imu.q[i]=imu.rf_quat[slave_id][i];

        //原始數據 pitch roll yaw，依序除以 100/100/10 轉換為角度
        imu.eul[0]=imu.rf_eul[slave_id][0]/100.0;
        imu.eul[1]=imu.rf_eul[slave_id][1]/100.0;
        imu.eul[2]=imu.rf_eul[slave_id][2]/10.0;

        for(int i=0;i<3;i++)
            imu.acc[i]=imu.rf_acc[slave_id][i];
        for(int i=0;i<3;i++)
            imu.gyr[i]=imu.rf_gyr[slave_id][i];
        for(int i=0;i<3;i++)
            imu.mag[i]=imu.rf_mag[slave_id][i];
        ShowOnTable(imu);
    }
    else if((ui->imu_btn->isChecked())){
        dump_imu_data(&imu);

        //原始數據 pitch roll yaw，依序除以 100/100/10 轉換為角度
        imu.eul[0]=imu.eul[0]/100.0;
        imu.eul[1]=imu.eul[1]/100.0;
        imu.eul[2]=imu.eul[2]/10.0;
        ShowOnTable(imu);
        if(QString::number(imu.id)!="")
            ui->imu_btn->setText("IMU(ID = "+QString::number(imu.id)+")");
        else
...
void MainWindow::ShowOnTable(imu_data_t imu)
{
    for(int i=0;i<4;i++){
        QString item= QString::number(imu.q[i],'f',3);
        QTableWidgetItem * protoitem = new QTableWidgetItem(item);
        protoitem->setTextAlignment(Qt::AlignCenter);
